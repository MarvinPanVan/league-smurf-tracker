...
test("opening the login panel records that the account was reached for", () => {
  ...
  assert.equal(win.filtered().find(a => a.id === "t4").lastPlayedAt, undefined,
    "opening Login is not the same as having played");
});

test("Play today queues playable accounts and sinks ones stamped played", () => {
  const now = Date.now();
  const day = 86400000;
  const seed = [
    { id: "flagged", gameName: "Flag", tagLine: "1", region: "EUW", status: "active", flagged: true,
      stats: { found: true, tier: "GOLD", division: "I", lp: 50, updatedAt: now }, history: [] },
    { id: "cold", gameName: "Cold", tagLine: "1", region: "EUW", status: "active",
      lastPlayedAt: now - 10 * day,
      stats: { found: true, tier: "SILVER", division: "I", lp: 10, updatedAt: now }, history: [] },
    { id: "played", gameName: "Done", tagLine: "1", region: "EUW", status: "active", fav: true,
      lastPlayedAt: now - 3600e3,
      stats: { found: true, tier: "PLATINUM", division: "I", lp: 10, updatedAt: now }, history: [] },
    { id: "rest", gameName: "Rest", tagLine: "1", region: "EUW", status: "resting",
      stats: { found: true, tier: "UNRANKED", division: null, lp: null, updatedAt: now }, history: [] },
    { id: "ban", gameName: "Ban", tagLine: "1", region: "EUW", status: "banned",
      stats: { found: true, tier: "BRONZE", division: "I", lp: 0, updatedAt: now }, history: [] },
    { id: "arch", gameName: "Arch", tagLine: "1", region: "EUW", status: "active", archived: true,
      stats: { found: true, tier: "GOLD", division: "II", lp: 0, updatedAt: now }, history: [] },
    { id: "fresh", gameName: "Fresh", tagLine: "1", region: "EUW", status: "active",
      stats: { found: true, tier: "IRON", division: "IV", lp: 0, updatedAt: now }, history: [] },
  ];
  const win = bootApp(seed);

  const tile = win.document.querySelector("[data-play-today]");
  assert.ok(tile, "Ready today appears when there are playable accounts");
  assert.match(tile.textContent, /Ready today/);
  assert.match(tile.textContent, /3/, "flagged + cold + fresh still need a stamp");

  win.document.getElementById("tPlay").click();
  assert.equal(win.document.getElementById("tPlay").getAttribute("aria-pressed"), "true");
  assert.match(win.document.getElementById("chips").textContent, /Play today/);

  const ids = Array.from(win.filtered().map(a => a.id));
  assert.deepEqual(ids.filter(id => ["rest", "ban", "arch"].includes(id)), [],
    "resting, banned and archived stay out of the queue");
  assert.equal(ids[0], "flagged", "flagged leads the queue");
  assert.ok(ids.indexOf("played") > ids.indexOf("fresh"), "already played today sinks below unplayed");
  assert.ok(ids.indexOf("played") > ids.indexOf("cold"));

  const card = () => win.document.querySelector('.card[data-id="fresh"]');
  assert.ok(card().querySelector('[data-act="played"]'), "Play mode puts Played on the foot");
  card().querySelector('[data-act="played"]').click();

  const stamped = JSON.parse(win.localStorage.getItem("smurf-tracker")).find(a => a.id === "fresh");
  assert.ok(stamped.lastPlayedAt && Date.now() - stamped.lastPlayedAt < 5000);
  assert.match(win.document.querySelector('.card[data-id="fresh"]').textContent, /Played/);
  assert.ok(win.playedToday(stamped));

  const after = Array.from(win.filtered().map(a => a.id));
  assert.ok(after.indexOf("fresh") > after.indexOf("cold"),
    "stamping moves it behind accounts still waiting");
  assert.equal(after[0], "flagged", "a flagged account still leads even after others are stamped");

  // Clear via the overflow menu still works when Play mode is off
  win.document.getElementById("tPlay").click();
  assert.equal(win.document.querySelector('.card[data-id="fresh"] [data-act="played"]'), null,
    "the foot shortcut is Play-mode only");
  win.document.querySelector('.card[data-id="fresh"] [data-act="more"]').click();
  win.document.querySelector('.card[data-id="fresh"] [data-act="played"]').click();
  assert.equal(JSON.parse(win.localStorage.getItem("smurf-tracker")).find(a => a.id === "fresh").lastPlayedAt,
    undefined, "Clear played today drops the stamp");
});

test("is:played and is:cold search the play stamps", () => {
  const now = Date.now();
  const seed = [
    { id: "today", gameName: "Today", tagLine: "1", region: "EUW", status: "active",
      lastPlayedAt: now - 1000,
      stats: { found: true, tier: "GOLD", division: "I", lp: 10, updatedAt: now }, history: [] },
    { id: "cold", gameName: "Cold", tagLine: "1", region: "EUW", status: "active",
      lastPlayedAt: now - 10 * 86400000,
      stats: { found: true, tier: "SILVER", division: "I", lp: 10, updatedAt: now }, history: [] },
    { id: "never", gameName: "Never", tagLine: "1", region: "EUW", status: "active",
      stats: { found: true, tier: "IRON", division: "IV", lp: 0, updatedAt: now }, history: [] },
    { id: "rest", gameName: "Rest", tagLine: "1", region: "EUW", status: "resting",
      stats: { found: true, tier: "UNRANKED", division: null, lp: null, updatedAt: now }, history: [] },
  ];
  ...
  const count = s => {
    ...
    win.renderFilters();
    return Array.from(win.filtered().map(a => a.id)).sort();
  };
  assert.deepEqual(count("is:played"), ["today"]);
  assert.deepEqual(count("is:cold"), ["cold", "never"], "cold skips resting and anything played this week");
  assert.deepEqual(count(""), ["cold", "never", "rest", "today"]);
});
