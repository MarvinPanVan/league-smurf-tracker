...
- **Organize**: favorites, **Play today** queue (stamp accounts after a session so they sink until tomorrow), tags, search, filters for region/status/tier/"never checked"/"needs refresh", sort by rank, name, level, LP change or last updated, archive accounts you're not using without deleting them, bulk actions (multi-select with shift-click range, set status/tag/delete for several at once).
